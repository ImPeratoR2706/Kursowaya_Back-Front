...
class AiTrainingRun(models.Model):
    """
    История запусков обучения/оценки AI-модуля.

    В рамках курсового проекта это фиксирует метрики качества и параметры,
    чтобы их можно было визуализировать во фронтенде.
    """

    created_at = models.DateTimeField("Дата запуска", default=timezone.now)
    ...
    model_version = models.CharField("Версия модели", max_length=100, default="")
    threshold = models.DecimalField("Порог классификации", max_digits=4, decimal_places=2, default=0.50)

    n_samples = models.PositiveIntegerField("Размер выборки", default=0)
    n_positive = models.PositiveIntegerField("Положительный класс (no_show)", default=0)

    accuracy = models.DecimalField("Accuracy", max_digits=6, decimal_places=4, null=True, blank=True)
    precision = models.DecimalField("Precision", max_digits=6, decimal_places=4, null=True, blank=True)
    recall = models.DecimalField("Recall", max_digits=6, decimal_places=4, null=True, blank=True)
    f1 = models.DecimalField("F1", max_digits=6, decimal_places=4, null=True, blank=True)

    tp = models.PositiveIntegerField("TP", default=0)
    fp = models.PositiveIntegerField("FP", default=0)
    tn = models.PositiveIntegerField("TN", default=0)
    fn = models.PositiveIntegerField("FN", default=0)
    ...
    def __str__(self) -> str:
        return f"AI training run {self.created_at:%Y-%m-%d %H:%M} ({self.model_version or 'unknown'})"
